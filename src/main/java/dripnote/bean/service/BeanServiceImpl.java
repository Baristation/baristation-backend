...
public class BeanServiceImpl implements BeanService {
    ...
    public ProductDetailDTO getProductDetail(Long productId) {
        ...
        // 2. 전체가 아닌, 이 특정 상품(productId)에 대한 데이터만 리포지토리에 요청
        // (기존의 Map 변환 과정을 생략하고 바로 List로 받습니다)
        ...
        // 3. 썸네일 이미지 추출 (정렬된 이미지 중 THUMB 타입 찾기)
        ProductImageDTO thumbImage = images.stream()
                .filter(img -> img.imageType() == ImageType.THUMB)
                .findFirst()
                .orElse(null);
