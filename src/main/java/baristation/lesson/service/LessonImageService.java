...
public interface LessonImageService {
     ...
     */
    LessonImageResponse uploadThumb(Long lessonId, MultipartFile file);

    /**
     * 클래스의 서브 이미지를 새롭게 추가합니다. (가장 마지막 순서로 배정)
     */
    LessonImageResponse uploadSub(Long lessonId, MultipartFile file);

    /**
     * 기존에 등록된 서브 이미지를 다른 이미지 파일로 교체합니다.
     */
    LessonImageResponse updateImage(Long lessonId, Long lessonImageId, MultipartFile file);
